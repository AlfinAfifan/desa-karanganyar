...
import { deleteByYear, deletePeraturan, getPeraturan } from '../../../redux/actions/peraturan/thunkPeraturan';
...
const TablePeraturan = () => {
  ...
  const isError = useSelector((state) => state.peraturan.error);
  const isSuccess = useSelector((state) => state.peraturan.deleteSuccess);
        ...
        <dialog id="modalOpsi" className="modal">
          <div className="modal-box w-3/12">
            ...
            <div className="flex justify-around mt-5">
              <Button onClick={() => ExportPeraturan(filteredData, year)} bgColor="bg-cyan-700" hoverBgColor="hover:bg-cyan-800">
            ...
            {filteredData
              ...
              .map((datafix, index) => (
                <Tr key={index}>
                  ...
                  <td>{datafix.tanggalPer ? formatNoTgl(datafix.tanggalPer, datafix.nomorPer) : ''}</td>
                  <td>{datafix.tentang}</td>
                  <td>{datafix.uraianSingkat}</td>
                  <td>{datafix.tanggalAcc ? formatNoTgl(datafix.tanggalAcc, datafix.nomorAcc) : ''}</td>
